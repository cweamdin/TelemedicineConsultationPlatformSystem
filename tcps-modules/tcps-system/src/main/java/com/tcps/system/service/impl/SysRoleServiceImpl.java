...
import com.tcps.system.domain.SysRoleMenu;
...
import com.tcps.system.mapper.SysRoleMenuMapper;
...
public class SysRoleServiceImpl implements ISysRoleService {
    ...
    public int authDataScope(SysRole role) {
        ...
        baseMapper.updateById(role);
        // 删除角色与部门关联
        roleOfficeMapper.delete(new LambdaQueryWrapper<SysRoleOffice>().eq(SysRoleOffice::getRoleId, role.getRoleId()));
        // 新增角色和部门信息（数据权限）
        return insertRoleOffice(role);
    }
     ...
     * @param role 角色对象
     */
    public int insertRoleOffice(SysRole role) {
        int rows = 1;
        // 新增角色与部门（数据权限）管理
        List<SysRoleOffice> list = new ArrayList<SysRoleOffice>();
        for (Long officeId : role.getOfficeIds()) {
            SysRoleOffice rd = new SysRoleOffice();
            rd.setRoleId(role.getRoleId());
            rd.setOfficeId(officeId);
            list.add(rd);
        }
        if (list.size() > 0) {
            rows = roleOfficeMapper.insertBatch(list) ? list.size() : 0;
        }
        return rows;
    }
    ...
    public int deleteRoleById(Long roleId) {
        ...
        // 删除角色与部门关联
        roleOfficeMapper.delete(new LambdaQueryWrapper<SysRoleOffice>().eq(SysRoleOffice::getRoleId, roleId));
    ...
    public int deleteRoleByIds(Long[] roleIds) {
        ...
        // 删除角色与部门关联
        roleOfficeMapper.delete(new LambdaQueryWrapper<SysRoleOffice>().in(SysRoleOffice::getRoleId, ids));
